...
def tag_chinese_with_translate_capability(text):
    ...
    if "eng" in it:
        ...
        for i in engWords:
            translatedText = text.replace(i, translate(i))
        
        r = get_token_tag(text)
        lengthAfterTranslate = len(r[0])
...
def getTokenTag(text):
    r = pos_tag(word_tokenize(text))
    return [x[0] for x in r],[x[1] for x in r]

udfTagEng = udf(lambda x : getTokenTag(x), ArrayType(ArrayType(StringType())))
