if __name__ == "__main__":
    ...
    app_name = "wikipedia chinese"
    data_path = "/home/pc/data/parsed_data/3parsed-extract-zhwiki.txt"
    ...
    from ChineseSentTokenize import *
    ...
    write_avro(bi, f"{hdfs_working_path}oriDataBC")
    # ################################################################################################################
    # ## preprocess data that will affect sentence tokenization
    df = read_avro(spark, f"{hdfs_working_path}oriDataBC")
    df = df.repartition(60)

    pp = pp(df)

    print("================ SAVE THE PREPROCESSED DATA ================")
    write_avro(pp, f"{hdfs_working_path}ppDataBC")
    remove_from_hdfs(f"{hdfs_working_path}oriDataBC")
    # ################################################################################################################
    # ## doing sentence tokenization on the dataset
    df = read_avro(spark, f"{hdfs_working_path}ppDataBC")
    df = df.repartition(60)
    ...
    write_avro(chiSent, hdfs_working_path+"bc_sent_tokenize.avro")
    remove_from_hdfs(f"{hdfs_working_path}ppDataBC")
    # ################################################################################################################
    ## preprocessing II
    df = read_avro(spark, f"{hdfs_working_path}bc_sent_tokenize.avro")
    df = df.repartition(60)

    p2 = preprocessing2(df, "text")

    print("================ SAVE THE CLEANED SENTENCE DATA ================")
    write_avro(p2, hdfs_working_path+"p2_bc")
    remove_from_hdfs(f"{hdfs_working_path}bc_sent_tokenize.avro")
    # ################################################################################################################
    # ## preprocessing III
    df = read_avro(spark, f"{hdfs_working_path}p2_bc")
    df = df.repartition(60)
    ...
    write_avro(a, f"{hdfs_working_path}final_process_bc")
    remove_from_hdfs(f"{hdfs_working_path}p2_bc")
    # ################################################################################################################
    ...
    df = read_avro(spark, f"{hdfs_working_path}final_process_bc")
    df = df.repartition(60)
    ...
    write_avro(filtered, f"{hdfs_working_path}filtered_bc_data")
    # ################################################################################################################
    # ## create sentence word count statistics
    df = read_avro(spark, f"{hdfs_working_path}filtered_bc_data")
    df = df.repartition(60)
    ...
    write_avro(df, hdfs_working_path+"bc_sentence_with_statistics")
    remove_from_hdfs(hdfs_working_path+"filtered_bc_data")
    # ################################################################################################################
    # ## remove the outliers
    s = read_avro(spark, hdfs_working_path+"bc_sentence_with_statistics")
    ...
    if want_remove_outliers:
        ...
        noS = s
    write_avro(noS, hdfs_working_path+"bc_sentence_without_out")
    remove_from_hdfs(hdfs_working_path+"bc_sentence_with_statistics")
    # ################################################################################################################
    # ## pos tag
    df = read_avro(spark, hdfs_working_path+"bc_sentence_without_out")
    df = df.repartition(60)
    chineseTag = df.withColumn("tag", udfTagChinese(df.text))

    chineseTag = chineseTag.rdd.map(lambda x : (x.text, x.tag[0], x.tag[1])).\
            toDF(["original","token","tag"])

    print("========== SENTENCES WITH TAGGING==========")
    write_avro(chineseTag, hdfs_working_path+"bc_sentence_with_tagging")
    # ################################################################################################################
    ## create n-gram
    a = read_avro(spark, hdfs_working_path+"bc_sentence_with_tagging")
    a = a.repartition(60)
    ...
    write_avro(a, hdfs_working_path+"bc_sentence_with_tagging_format")
    remove_from_hdfs(hdfs_working_path+"bc_sentence_with_tagging")
    # ################################################################################################################

    a = read_avro(spark, hdfs_working_path+"bc_sentence_with_tagging_format")
    a = a.repartition(60)
    ...
    .select("sentence_id", "original", "tag","token", posexplode(col("tmp")))\
    .select("sentence_id", "original", "tag", "token", "pos", col("col.token_gram"), col("col.tag_gram"))

    print("==========CREATED BIGRAM==========")
    write_avro(t, hdfs_working_path+"BC_N2_GRAM")
    # ################################################################################################################
    # ## trigram
    ...
    write_avro(t, hdfs_working_path+"BC_N3_GRAM")
    # ################################################################################################################
    # ## n4 gram
    ...
    write_avro(t, hdfs_working_path+"BC_N4_GRAM")
    # ################################################################################################################
    # ## n5 gram
    ...
    write_avro(t, hdfs_working_path+"BC_N5_GRAM")
    # ################################################################################################################
    # ## merge ngram
    n2 = read_avro(spark, hdfs_working_path+"BC_N2_GRAM")
    n3 = read_avro(spark, hdfs_working_path+"BC_N3_GRAM")
    n4 = read_avro(spark, hdfs_working_path+"BC_N4_GRAM")
    n5 = read_avro(spark, hdfs_working_path+"BC_N5_GRAM")
    ...
    write_avro(merge, hdfs_working_path+"BC_MERGE_GRAM")
    # ################################################################################################################
    # ## gram frequency statistics
    # ## Tag frequency
    # ################################################################################################################
    # ## filter n gram with key words
    df = read_avro(spark, hdfs_working_path+"BC_MERGE_GRAM")
    df = df.repartition(60)
    keys = set(keys)
    ...
    write_avro(withKeyOnly, f"{hdfs_working_path}BC_KeyOnlyData")
    # ################################################################################################################
    # # create the column to identify whether keyword at middle
    df = read_avro(spark,  f"{hdfs_working_path}BC_KeyOnlyData")
    df = df.repartition(60)
    ...
    write_avro(withMiddleFlag, f"{hdfs_working_path}BC_MiddleFlagData")
    remove_from_hdfs(f"{hdfs_working_path}BC_KeyOnlyData")
    # ################################################################################################################
    # ## Frequency statistics
    # ## tag frequency
    df = read_avro(spark,  f"{hdfs_working_path}BC_MiddleFlagData")
    df = df.repartition(60)
    tagFreq = df.groupBy("tag_gram").count().withColumnRenamed("count", "tag_gram_f")
    write_avro(tagFreq, hdfs_working_path+"BC_TAG_FREQ")
    tagFreq = read_avro(spark, hdfs_working_path+"BC_TAG_FREQ")
    withtagFBI = df.join(tagFreq, df.tag_gram == tagFreq.tag_gram).drop(tagFreq.tag_gram)

    print("==========COMPUTED THE TAG FREQUENCY==========")
    write_avro(withtagFBI, hdfs_working_path+"BC_TAG_F")
    remove_from_hdfs(hdfs_working_path+"BC_TAG_FREQ")
    # ################################################################################################################
    # ## Token frequency
    df = read_avro(spark, hdfs_working_path+"BC_TAG_F")
    df = df.repartition(60)
    bitokenFreq = df.groupBy("token_gram").count().withColumnRenamed("count", "token_gram_f")
    write_avro(bitokenFreq, hdfs_working_path+"BC_TOKEN_FREQ")
    tokenFreq = read_avro(spark, hdfs_working_path+"BC_TOKEN_FREQ")
    ...
    write_avro(biwithTokF, hdfs_working_path+"BC_TOKEN_F")
    remove_from_hdfs(hdfs_working_path+"BC_TOKEN_FREQ")
    remove_from_hdfs(hdfs_working_path+"BC_TAG_F")
    # ################################################################################################################
    ## Table Normalization
    ## Gram table
    df = read_avro(spark, hdfs_working_path+"BC_TOKEN_F")
    ...
    write_avro(gram_table, hdfs_working_path+"BC_GRAM_TABLE")
    # ################################################################################################################
    ## Source Table
    ...
    write_avro(originalSentence, hdfs_working_path+"BC_SOURCE_TABLE")
    # ################################################################################################################
    ## Into Hive
    gram_table = read_avro(spark, hdfs_working_path+"BC_GRAM_TABLE")
    original_table = read_avro(spark, hdfs_working_path+"BC_SOURCE_TABLE")

    spark.sql("create database if not exists wikipedia_db;")
    spark.sql("use wikipedia_db;")
    spark.sql("drop table if exists gram_table_zh;")
    spark.sql("drop table if exists source_data_zh;")


    gram_table.write\
    .format("orc") \
    .mode("overwrite") \
    .saveAsTable("wikipedia_db.gram_table_zh")


    original_table.write\
    .format("orc") \
    .mode("overwrite") \
    .saveAsTable("wikipedia_db.source_data_zh")
    ...
    ax.barh(x,y)
    ax.set_title('Top 10 Most Common POS Patterns in Wikipedia Chinese')
    ...
    plt.savefig('visualization/top10zhWIKIPOSPattern.png', dpi=300, bbox_inches='tight')
    ################################################################################################################
    ## Analytics
    ...
    ax.barh(x,y)
    ax.set_title('Top 10 Most Common Token Patterns in Wikipedia Chinese')
    for index, value in enumerate(y):
        ax.text(value, index,
                 str(value))
    ax.invert_yaxis()

    plt.savefig('visualization/top10zhWIKITokenPattern.png', dpi=300, bbox_inches='tight')
